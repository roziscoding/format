...
describe('Format', () => {
  ...
  it('Ignores non-formatable params', () => {
    expect(format`Hello, ${name}! You are ${age} years old!`).to.equal('Hello, Roz! You are 23 years old!')
  })

  it('Formats all formatable params', () => {
    expect(format`Date: ${{ date: dateFixture }}. Time: ${{ time: dateFixture }} :)`).to.equal('Date: 01/01/2020. Time: 08:00:00 :)')
  })

  it('Formats mixed params', () => {
    expect(format`Name: ${name}, age: ${age}, Date: ${{ date: dateFixture }}, Time: ${{ time: dateFixture }} :)`)
